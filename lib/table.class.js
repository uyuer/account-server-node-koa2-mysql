...
class Table {
    constructor(table) {
        this.tableIns = table;
        this.dbName = table.getSchema().getName()
    ...
    static async build(tableName) {
        try {
            console.count('创建session', session)
            // let ins = await mysqlx.getSession({
            //     host: database.HOST,
            //     port: database.PORT,
            //     user: database.USERNAME,
            //     password: database.PASSWORD,
            // }).then(s => s.getSchema(database.DATABASE))
            let ins = await session.then(s => s.getSchema(database.DATABASE));
            let table = ins.getTable(tableName);
            return new Table(table)
        } catch (error) {
            console.log('这里报错', error)
        }
    }
     ...
     * @return {Promise}
     */
    async find(where, selects = ['id'], orderBy) {
        let table = this.tableIns;
        let finder = table
            .select(...selects)
            .where(where)
        orderBy && finder.orderBy(orderBy)
     ...
     */
    async findOne(where, selects = ['id'], orderBy) {
        let finder = await this.find(where, selects, orderBy);
     ...
     */
    async findMultiple(where, selects = [], pageNum = 1, pageSize = 10, orderBy) {
        let finder = await this.find(where, selects, orderBy);
     ...
     * @return {array} 列表
     */
    async findAll(where, selects = [], orderBy) {
        let finder = await this.find(where, selects, orderBy);
