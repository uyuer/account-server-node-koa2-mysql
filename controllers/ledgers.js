...
const details = {
	...
	// 查找-指定ID查找账本信息
	findOne: async (ctx) => {
		console.log(`请求->账单详情->查询数据详细信息: ledgers.details.findOne; method: ${ctx.request.method}; url: ${ctx.request.url} `)
		let params = ctx.verifyParams({
			id: [{ required: true, message: "账本id不可为空" }, { pattern: /^[1-9]\d*$/, message: "账本id格式错误" }],
		})
		let { userId } = ctx.session.user || {};
		...
		// 这里使用了userId辅助精确删除, 但是当admin等管理员删除时会存在问题, 可能需要做一个权限的判断之类的
		let { id } = params;
		let where = `bookId=${id} and userId=${userId}`;
		let selects = []; // 查询全部
		let result = await ledgersDetailsTable.findOne(where, selects);
		let statusText = formatStatus(result.status);
		ctx.bodys = { ...result, statusText };
	},
	// 查找-多条数据(根据userId找到该用户下的账本数据, 可分页)
	findMultiple: async (ctx) => {
		console.log(`请求->账单详情->查询用户的账本列表: ledgers.details.findMultiple; method: ${ctx.request.method}; url: ${ctx.request.url} `)
		let params = ctx.verifyParams({
			pageNum: [{ required: false, message: "" }, { pattern: /^[1-9]\d*$/, message: "pageNum格式错误" }],
			pageSize: [{ required: false, message: "" }, { pattern: /^[1-9]\d*$/, message: "pageSize格式错误" }],
		})
		...
		let where = `userId=${userId}`;
		let selects = [];
		let result = await ledgersDetailsTable.findMultiple(where, selects, pageNum, pageSize);
		ctx.bodys = result;
	},
	// 查找-全部数据(根据userId找到该用户下的全部账本数据)
	findAll: async (ctx) => {
		...
		// ---分隔线---
		let { ledgersDetailsTable } = await instanceTable();
		let result = await ledgersDetailsTable.findAll(`userId=${userId}`);
		ctx.bodys = result;
	},
	// 更新一条账本信息
	updateOne: async (ctx) => {
